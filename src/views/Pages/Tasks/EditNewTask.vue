                      ...
                      <v-row dense justify="center">
                        ...
                        <v-col cols="12" md="6">
                          <v-autocomplete
                            ...
                            item-value="id"
                            dense
                            v-model="defaultTask.assignedEmps"
                            :items="employees"
                          ></v-autocomplete>
                        </v-col>
                        <v-col cols="12" md="11">
...
export default {
  ...
  computed: {
   ...
   watch: {
     getSelectedTask (val) {
      this.defaultTask.id = this.getSelectedTask.id;
      this.defaultTask.taskType = this.getSelectedTask.taskType;
      this.defaultTask.parentTask = this.getSelectedTask.parentTask;
      this.defaultTask.parentIncident = this.getSelectedTask.parentIncident;
      this.defaultTask.isIncident = this.getSelectedTask.isIncident;
      this.defaultTask.description = this.getSelectedTask.description;
      this.defaultTask.title = this.getSelectedTask.title;
      this.defaultTask.urgent = this.getSelectedTask.urgent;
      this.defaultTask.importance = this.getSelectedTask.importance;
      this.defaultTask.status = this.getSelectedTask.status;
      this.defaultTask.assigned_for = this.getSelectedTask.assigned_for;
      this.getSelectedTask.assignedEmps.map(x => x.eUserId).forEach(x => this.defaultTask.assignedEmps.push(x))
      this.defaultTask.weights = this.getSelectedTask.weight;
      this.defaultTask.actions = this.getSelectedTask.actions;
      this.defaultTask.date = this.getSelectedTask.date;
      this.Attachments = this.getSelectedTask.attachments
     }
   },
  methods: {
    ...mapActions("departments", ["fetchDepartments"]),
    ...mapActions("tasks", ["UpdateTask", "fetchTaskType", "fetchIncidentTasks"]),
    ...
    add() {
      ...
      if (this.defaultTask.assignedEmps.length == 0) {
        isValid = false;
      ...
      if (!isValid) {
        ...
        this.defaultTask.assignedEmps = this.defaultTask.assignedEmps.map((x) => ({
          euser: { id: x },
        }));


        this.UpdateTask(this.defaultTask)
          .then((rep) => {
