            ...
            <v-col cols="12" md="12" style="margin-left: 200px">
              <v-simple-table dense>
                <template v-slot:default>
                  <thead>
                    <tr>
                      <th class="text-right">إسم الأداة</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(item,i) in tools" :key="item.name">
                      <td>{{ item.name }}</td>
                      <td>
...
export default {
  ...
  computed: {
    ...mapGetters("apt", ["gettoolName"])
  },

  // get the actions from the store
  methods: {
    ...mapActions("apt", ["AddtoolName", "removetoolName"]),
    sendData() {
      let found = this.tools.some(({name}) => name == this.tool);

      if (found) {
        return this.statAlert(
          true,
          "( " + this.tool.name + " )" + "تم إضافته مسبقا  ",
          "error"
        );
      } else {
       this.tools.push({ name :  this.tool });
       this.tool= "";
      }
    },
    close(index) {
      this.tools.splice(index);
    },
