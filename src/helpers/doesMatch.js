...
import * as _ from "lodash";
export function doesItemMatch(item, filterFieldName, filter) {
...
function doesManyMatch(itemValue, filterValue) {
    ...
    const parsedFilterValueKeys = new Set(parsedFilterValue);
    let matches = parsedValue.map((str) => parsedFilterValueKeys.has(str));
    const doesMatch = matches.reduce((acc, match) => acc || match, false);
    return doesMatch;
}
function doesStringMatch(itemValue, filterValue, optionsParsed) {
    ...
    if (!optionsParsed.caseSensitive) {
        parsedItemValue = parsedItemValueInput.toLowerCase();
        parsedFilter = parsedFilterInput.map((i) => (i + "").toString().toLowerCase());
    }
    let matches = parsedFilter.map((str) => str === parsedItemValue);
    const doesMatch = matches.reduce((acc, match) => acc || match, false);
    return doesMatch;
}
